...
|------|----------|-----------|
| `s` | 垂直分割で開く | **(無効化済み)** グローバルの`s`プレフィックス用に解放 |
| `p` | 親ディレクトリへ移動 | **貼り付け** |
| `x` | 親を閉じる | **カット** |
...
バッファローカルのマップはグローバルより優先され、しかも**完全一致した時点で即発火する**(`timeoutlen`は待たない)。
そのため以前はツリー内で`s`を押した瞬間にツリー側の`s`(既定アプリで開く)が確定し、`sf`等が一切効かなかった。
`on_attach`で衝突するキーだけ削除して解消している。

- **`s`系(`sf` `so` `sj`等25キー)はツリー内でも通常バッファと同じに動く**
- **`Ctrl+hjkl`のウィンドウ移動もツリー内で動く**(ツリー側の`Ctrl+k`=ファイル情報の表示を削除した)
- 代償として、ツリー内の**「既定アプリで開く」(`s`)と「ファイル情報の表示」(`Ctrl+k`)は使えない**。前者は`.`(任意コマンド実行)で代替できる
- **`x` `Y` `M` `U` `f` `c` `d` `y` `e` `q`はツリー側が優先のまま**。ツリーは編集不可バッファでグローバル側(`"_x`や`y$`や`%`)が無意味なため、あえて残している
